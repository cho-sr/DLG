...
def parse_layer_sparsity_rules(rule_text):
    if not rule_text:
        return [], None

    rules = []
    default_ratio = None
    for raw_item in rule_text.split(","):
        item = raw_item.strip()
        if not item:
            continue
        if "=" not in item:
            raise ValueError(f"Invalid layer sparsity rule '{item}'. Use 'pattern=ratio'.")
        key, value = item.split("=", 1)
        key = key.strip()
        try:
            ratio = float(value.strip())
        except ValueError as exc:
            raise ValueError(f"Invalid ratio in rule '{item}'.") from exc
        if ratio <= 0.0 or ratio > 1.0:
            raise ValueError(f"Ratio in rule '{item}' must be in (0.0, 1.0].")
        if key == "default":
            default_ratio = ratio
        else:
            rules.append((key, ratio))

    return rules, default_ratio


def sparsify_gradients_layerwise_topk(gradients, param_names, base_ratio, layer_rules, default_ratio=None):
    if len(gradients) != len(param_names):
        raise ValueError("Length mismatch: gradients and param_names.")
    if base_ratio <= 0.0 or base_ratio > 1.0:
        raise ValueError("base_ratio must be in (0.0, 1.0].")
    if default_ratio is not None and (default_ratio <= 0.0 or default_ratio > 1.0):
        raise ValueError("default_ratio must be in (0.0, 1.0].")
    ...
    for name, g in zip(param_names, gradients):
        ratio = default_ratio if default_ratio is not None else base_ratio
        for pattern, pattern_ratio in layer_rules:
            if pattern in name:
                ratio = pattern_ratio
                break
        ...
        per_layer_stats.append(
            {
                ...
                "kept": kept,
                "total": total,
                "retention_ratio": kept / float(max(1, total)),
    ...
    stats = {
        "kept": kept_total,
        "total": total_all,
        "retention_ratio": kept_total / float(max(1, total_all)),
...
def parse_args():
    p = argparse.ArgumentParser("FedAvg + DLG (layer-wise sparsification)")
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Layer-wise retention rules, e.g. 'conv=0.2,fc=0.05,default=1.0'. "
            "If set, top-k sparsification is applied per parameter tensor using matching name patterns."
        ),
    )
...
def main():
    ...
    if args.layer_sparsity:
        print(f"  layer_sparsity={args.layer_sparsity}")
    ...
    for rnd in range(args.rounds):
        ...
        for cid in selected:
            ...
            if rnd == args.attack_round and cid == args.victim_client and captured_grad is None:
                ...
                if args.layer_sparsity:
                    layer_rules, default_layer_ratio = parse_layer_sparsity_rules(args.layer_sparsity)
                    param_names = [name for name, _ in net.named_parameters()]
                    captured_grad, sparse_stats = sparsify_gradients_layerwise_topk(
                        gradients=captured_grad,
                        param_names=param_names,
                        base_ratio=args.sparsity,
                        layer_rules=layer_rules,
                        default_ratio=default_layer_ratio,
                    )
                else:
                ...
                if "per_layer" in sparse_stats:
                    for layer_stat in sparse_stats["per_layer"]:
                        print(
                            "    - "
                            f"{layer_stat['name']}: kept {layer_stat['kept']}/{layer_stat['total']} "
                            f"({layer_stat['retention_ratio'] * 100:.2f}%, configured={layer_stat['configured_ratio']})"
                        )
...
if __name__ == "__main__":
    main()
#reorganing fedavg and waiting retest