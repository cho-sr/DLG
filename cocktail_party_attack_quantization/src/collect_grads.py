...
def collect_grads(args):
    ...
    if args.defense == "quant":
        exp_path = f"{exp_path}/q_bits_{args.q_bits}"
    ...
    if args.defense == "quant":
        logger.info(f"Applying symmetric gradient quantization with q_bits={args.q_bits}")
    device = get_device()
    ...
    for batch_size in batch_size_list:
        ...
        for i, (x, y) in enumerate(dl_test):
            ...
            if args.defense == "quant":
                grad = quantize_gradients(grad, q_bits=args.q_bits)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="number of bits for symmetric gradient quantization",
    )

    args = parser.parse_args()
    if args.q_bits < 2:
        parser.error("--q_bits must be at least 2")
