...
def get_grads_file(ds, model, defense, batch_size, C, sigma, q_bits=8):
    exp_path = f"{exp_path_base}/{ds}/{model}/grads/{defense}"
    if defense == "nodef":
        grads_file = f"{exp_path}/{batch_size}.pickle"
    elif defense == "quant":
        grads_file = f"{exp_path}/q_bits_{q_bits}/{batch_size}.pickle"
    else:
...
def quantize_tensor_symmetric(tensor, q_bits=8):
    if q_bits < 2:
        raise ValueError("q_bits must be at least 2 for signed quantization")
...
def quantize_gradients(grad_list, q_bits=8):
    return [quantize_tensor_symmetric(grad, q_bits=q_bits) for grad in grad_list]
...
def get_attack_exp_path(args):
    if args.exp_name is not None:
        ...
        elif args.defense == "quant":
            exp_name = f"q_bits_{args.q_bits}_decor_{args.decor}_t_{args.T}_tv_{args.tv}_nv_{args.nv}_l1_{args.l1}"
        else:
            exp_name = (
