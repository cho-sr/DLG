...
def get_grads_file(
    ...
    if defense == "nodef":
        ...
        sparsity_name = format_ratio(sparsity)
        grads_file = (
            f"{exp_path}/{sparsify_method}/sparsity_{sparsity_name}/{batch_size}.pickle"
        )
    else:
...
def validate_sparsity(sparsity):
    if sparsity <= 0.0 or sparsity > 1.0:
        raise ValueError("sparsity must be in the range (0.0, 1.0]")


def sparsify_gradients_global_topk(grad_list, sparsity=0.1):
    validate_sparsity(sparsity)
    if sparsity >= 1.0:
        return [grad.clone() for grad in grad_list]

    flat = torch.cat([grad.reshape(-1) for grad in grad_list])
    total = flat.numel()
    kept = max(1, int(total * sparsity))
    ...
    for grad in grad_list:
        n = grad.numel()
        sparse_grads.append(sparse_flat[offset : offset + n].view_as(grad))
        offset += n

    return sparse_grads


def sparsify_gradients_layerwise_topk(grad_list, sparsity=0.1):
    validate_sparsity(sparsity)
    if sparsity >= 1.0:
        return [grad.clone() for grad in grad_list]
    ...
    for grad in grad_list:
        ...
        mask = torch.zeros_like(flat)
        mask[topk_idx] = 1.0
        sparse_grads.append((flat * mask).view_as(grad))

    return sparse_grads


def sparsify_gradients(grad_list, method="global_topk", sparsity=0.1):
    if method == "global_topk":
        return sparsify_gradients_global_topk(grad_list, sparsity=sparsity)
    elif method == "layerwise_topk":
        return sparsify_gradients_layerwise_topk(grad_list, sparsity=sparsity)
    else:
...
def get_attack_exp_path(args):
    if args.exp_name is not None:
        ...
        if args.defense == "dp":
            ...
            sparsity_name = format_ratio(args.sparsity)
            exp_name = (
                f"{args.sparsify_method}_sparsity_{sparsity_name}_decor_{args.decor}"
                f"_t_{args.T}_tv_{args.tv}_nv_{args.nv}_l1_{args.l1}"
            )
        else:
            exp_name = (
