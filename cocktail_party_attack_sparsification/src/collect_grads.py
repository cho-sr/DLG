...
def collect_grads(args):
    ...
    if args.defense == "sparse":
        sparsity_name = format_ratio(args.sparsity)
        exp_path = f"{exp_path}/{args.sparsify_method}/sparsity_{sparsity_name}"
    ...
    if args.defense == "sparse":
        logger.info(
            "Applying gradient sparsification with "
            f"method={args.sparsify_method}, sparsity={args.sparsity}"
        )
    device = get_device()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="gradient sparsification method",
    )
    parser.add_argument(
        "--sparsity",
        type=float,
        default=0.1,
        help="gradient retention ratio for sparsification; 0.1 keeps 10 percent",
    )

    args = parser.parse_args()
    if args.sparsity <= 0.0 or args.sparsity > 1.0:
        parser.error("--sparsity must be in the range (0.0, 1.0]")
